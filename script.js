...
const initCodeEditor = () => {
    ...
    statsEl = byId('stats');
    editor.on('change', () => {
        statsEl.innerHTML = `Lines: ${editor['doc'].size} |  Words: ${countWords(editor['doc'].getValue())} |  Characters: ${editor.getValue().length}`;
        hideCopyBar();
    });
};
...
const initLangSelector = () => {
    select = new SlimSelect({
        ...
        onChange: (e) => {
            ...
            document.title = e.text && e.text !== 'Plain Text' ? `Bin | ${e.text}` : 'Bin';
        },
    });
...
const buildUrl = (rawData, mode) => {
    ...
    if (mode === 'markdown') {
        return `[KyleTheUndead | BIN - snippet](${url})`;
    }
    if (mode === 'iframe') {
